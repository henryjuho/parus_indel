...
prox_data = subset(read.csv('gt_sel_neu_ref_genedist.results.csv'), bin!='10-16')
...
prox_data$distance = as.numeric(prox_data$bin) * 50000
...
cor.test(as.numeric(rdi_data$rdi), rdi_data$distance, method='spearman', exact=NULL)

theta_plot = ggplot(subset(prox_data, sel_type=='sel'), aes(x=distance, y=theta, colour=var_type)) +
      geom_point(stat='identity', size = 2) +
      theme_bw() + geom_smooth(method='lm') +
      scale_colour_manual(values=cbPalette) +
      ylab(expression(theta))  + xlab('Distance from gene (bp)') +
      theme(legend.title = element_blank(), legend.position = c(0.8, 0.6), legend.background = element_blank())

gamma_plot = ggplot(subset(prox_data, sel_type=='sel'), aes(x=distance, y=mean_gamma, colour=var_type)) +
      geom_point(stat='identity', size = 2) +
      theme_bw() +
      scale_colour_manual(values=cbPalette) +
      ylab(expression(gamma))  + xlab('Distance from gene (bp)') +
...
rdi_plot = ggplot(rdi_data, aes(x=distance, y=rdi)) +
      geom_point(stat='identity', size = 2) +
      theme_bw() +
      ylab('rDI')  + xlab('Distance from gene (bp)')
