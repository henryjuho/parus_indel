...
prox_data = read.csv('gt_sel_neu_ref_genedist_5kb.results.csv')
...
theta_plot = ggplot(subset(prox_data, sel_type=='sel'), aes(x=as.numeric(bin), y=theta, colour=var_type)) +
      geom_point(stat='identity', size = 2) +
      theme_bw() +
      scale_colour_manual(values=cbPalette) +
      ylab(expression(theta))  + xlab('Distance from gene / 5kb') +
      theme(legend.title = element_blank(), legend.position = c(0.15, 0.8), legend.background = element_blank())

gamma_plot = ggplot(subset(prox_data, sel_type=='sel'), aes(x=as.numeric(bin), y=mean_gamma, colour=var_type)) +
      geom_point(stat='identity', size = 2) +
      theme_bw() +
      scale_colour_manual(values=cbPalette) +
      ylab(expression(gamma))  + xlab('Distance from gene / 5kb') +
      theme(legend.title = element_blank(), legend.position = 'none', legend.background = element_blank())

rdi_plot = ggplot(rdi_data, aes(x=as.numeric(bin), y=rdi)) +
      geom_point(stat='identity', size = 2) +
      theme_bw() +
      geom_smooth(method='lm') +
      ylab('rDI')  + xlab('Distance from gene / 5kb')

png(file='distance_estimates_5kb.png', width=9, height=3, units='in', res=360)
