...
in_file = 'gt_windows_noncoding_discrete_full_results.windowdata.csv'
out_stem = 'discrete'
...
window_data_sel = subset(window_data, sel_type=='sel' & converged=='True' & boundaries_hit!='sel_del_gamma_1|sel_ins_e_1')
...
# gamma correlations
gamma_ins_test = cor.test(ins_data$gamma, ins_data$rec_rate, method='spearman', exact=NULL)
gamma_del_test = cor.test(del_data$gamma, del_data$rec_rate, method='spearman', exact=NULL)
