...
parser.add_argument('-lrt', '--likelihood_ratio_test', help='Paramter to fix for likelihood ratio test',
                    choices=['gamma_indel', 'gamma_ins', 'gamma_del', 'kappa'], required=True)
...
for x in ['none', lrt]:
    ...
    for alg in algorithm_list:
        ...
        q_sub([anavar_cmd], out_prefix + '.' + reduction[0] + '.' + algorithm.lower(),
              evolgen=args.evolgen, t=48, jid=jid)

# construct merged ml output
ml_out = out_prefix + '.maxL.txt'
with open(ml_out, 'w') as mlo:
    mlo.write('model\talgorithm\tsnp_theta\tsnp_error\tsnp_gamma\tindel_theta\tindel_kappa\tinsertion_gamma\t'
              'deletion_gamma\tinsertion_error\tdeletion_error\tlnL\tp_value\n')
...
q_sub(cmd_list, out_prefix + '.ml_best.ratiotest', hold=jids)
